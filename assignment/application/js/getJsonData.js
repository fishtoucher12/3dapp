...
$(document).ready(function(){
	 ...
	 $.getJSON('application/model/data.json', function(jsonObj) {
		...
		$('.x3dCameraSubtitle').html('<h5>' + jsonObj.pageTextData[8].CameraTitle + '</h5>');
		$('.x3dCameraDescription').html('<p>' + jsonObj.pageTextData[8].CameraSubtitle + '</p>');

		//Get light contents data
        $('.x3dLightSubtitle').html('<h5>' + jsonObj.pageTextData[9].LightSubtitle + '<h5>');
		$('.x3dLightDescription').html('<p>' + jsonObj.pageTextData[9].LightDescription + '</p>');

		$('.x3dAnimationDescription').html('<p>' + jsonObj.pageTextData[10].AnimationDescription + '</p>');
